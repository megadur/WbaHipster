...
import com.dtit.tm.config.KafkaProperties;
...
class WbaHipsterKafkaResourceIT {
    ...
    void setup() {
        ...
        WbaHipsterKafkaResource kafkaResource = new WbaHipsterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/wba-hipster-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/wba-hipster-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
