...
/**
 * Performance test for the BngInfo entity.
 */
class BngInfoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BngInfo entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all bngInfos")
            .get("/api/bng-infos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new bngInfo")
            .post("/api/bng-infos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "endSz":"SAMPLE_TEXT"
                , "lineId":"SAMPLE_TEXT"
                , "portName":"SAMPLE_TEXT"
                , "tsLastQuery":"SAMPLE_TEXT"
                , "tsLastQuerySuccess":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_bngInfo_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created bngInfo")
                .get("${new_bngInfo_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created bngInfo")
            .delete("${new_bngInfo_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
