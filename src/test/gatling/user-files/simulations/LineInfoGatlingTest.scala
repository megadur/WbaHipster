...
/**
 * Performance test for the LineInfo entity.
 */
class LineInfoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the LineInfo entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all lineInfos")
            .get("/api/line-infos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new lineInfo")
            .post("/api/line-infos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "lineId":"SAMPLE_TEXT"
                , "uplinkPort":"SAMPLE_TEXT"
                , "ipAddress":"SAMPLE_TEXT"
                , "tsLastQuery":"SAMPLE_TEXT"
                , "tsLastQuerySuccess":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_lineInfo_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created lineInfo")
                .get("${new_lineInfo_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created lineInfo")
            .delete("${new_lineInfo_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
